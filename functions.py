...
class UserService:
    ...
    def save_additional_details(self, username, fullname, contactNo, address, reference_no):
        """Store payment details in the payment_details table."""
        conn = self.db.get_connection()
        cursor = conn.cursor()

        cursor.execute(
            """
            INSERT INTO payment_details (username, fullname, address, contact_no, reference_no)
            VALUES (%s, %s, %s, %s, %s)
            """,
            (username, fullname, address, contactNo, reference_no),
        )
...
def save_additional_details(username, fullname, contactNo, address, reference_no):
    return _user_service.save_additional_details(username, fullname, contactNo, address, reference_no)
